...
pub fn buy_pico() {
    ...
    if amount >= 5000 {
        if Path::new(".objects/items/pico").exists() == false {
            add_pico().unwrap();
...
pub fn add_pico() -> std::io::Result<()> {
    let mut bal = fs::File::create(".objects/items/pico")?;
    bal.write_all(b"1")?;
    Ok(())
}

pub fn get_pico_lvl() -> u32 {
    let dir = ".objects/items/pico";
...
pub fn mine() {
    if Path::new(".objects/items/pico").exists() == true {
        let lvl = get_pico_lvl();
